...
$nzshpcrt_gateways[$num] = array(
	'name' => __( 'Bank', 'wpsc' ),
	'api_version' => 2.0,
	'class_name' => 'wpsc_merchant_bank',
	'has_recurring_billing' => true,
	'display_name' => __( 'Bank', 'wpsc' ),
...
function form_bank() {
	$output = "
		<tr>
			<td>
				" . __( 'Payment Instructions_bank', 'wpsc' ) . "
			</td>
			<td>
				".__('Enter the payment instructions that you wish to display to your customers when they make a purchase', 'wpsc')."
				<textarea cols='40' rows='9' name='wpsc_options[bank_instructions]'>" . esc_textarea( get_option( 'bank_instructions' ) ) . "</textarea><br />
				<p class='description'>
...
function _wpsc_filter_bank_merchant_customer_notification_raw_message( $message, $notification ) {
	$purchase_log = $notification->get_purchase_log();

	if ( $purchase_log->get( 'gateway' ) == 'wpsc_merchant_bank' )
		$message = get_option( 'bank_instructions', '' ) . "\r\n" . $message;
...
add_filter(
	...
	'_wpsc_filter_bank_merchant_customer_notification_raw_message',
	10,
	2
);
add_filter(
